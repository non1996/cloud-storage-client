...
//				controller->PushPutCommand(param[1], param[2], param[3]);	//文件名,文件路径，上传到的位置
				controller->PushPutCommand("fuli.wmv", "D:\\", "/");
			}
			else {
				std::cout << "wrong input\n";
			}
			...
			if (param.size() == 2) {
				controller->PushDeleteCommand(param[1], std::string("turpis"));
			}
			else {
				std::cout << "wrong input\n";
			}
			...
			if (param.size() >= 3) {
				std::vector<std::string> args(param.begin() + 3, param.end());	//std::string("0")
				controller->PushLsCommand(param[1], std::string(""), param[2], args);
			}
			else {
				std::cout << "wrong input\n";
			}
			...
			if (param.size() == 3) {
				controller->PushMkdirCommand(std::string("myDocument"), std::string("/"));
			}
			else {
				std::cout << "wrong input\n";
			}
			...
			if (param.size() == 2) {
				controller->PushCopyCommand(param[1], std::string("/myDocument/"));
			}
			else {
				std::cout << "wrong input\n";
			}
			...
			if (param.size() == 4) {
				std::cout << param[1] << param[2] << param[3] << std::endl;
